<template>
    <view class="demo-doc demo-password-input">
        <demo-block
            ref="basicUsage"
            :title="t('basicUsage')">
            <van-password-input
...
const current = ref('basicUsage')
const errorInfo = ref('')
const showInfo = ref()
const addGutter = ref()
const basicUsage = ref()
const removeMask = ref()
...
const onDelete = () => {
    ...
    values.value[current.value] = values.value[current.value].slice(0, values.value[current.value].length - 1)
